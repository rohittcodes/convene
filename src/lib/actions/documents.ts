...
import { addRelation, removeRelation } from '@/lib/fga/fga';
import { auth0 } from '@/lib/auth0';

export const createDocument = async (input: NewDocumentParams, text: string): Promise<DocumentParams> => {
  ...
  if (embeddings.length > 0) {
    ...
    await addRelation(user.email!, `document:${document.id}`, 'owner');
  }
...
export async function shareDocument(documentId: string, sharedWith: string[]) {
  ...
  for (const user of sharedWith) {
    await addRelation(user, `document:${documentId}`, 'viewer');
  }
}
...
export async function deleteDocument(documentId: string) {
  ...
  await removeRelation(user.email!, `document:${documentId}`, 'owner');
  const currentSharedWith = await db
    .select({ sharedWith: documentsTable.sharedWith })
    .from(documentsTable)
    .where(eq(documentsTable.id, documentId));
  ...
  for (const sUser of currentSharedWith[0]?.sharedWith) {
    await removeRelation(sUser, `document:${documentId}`, 'viewer');
  }
